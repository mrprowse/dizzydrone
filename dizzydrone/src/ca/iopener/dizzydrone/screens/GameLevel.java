...
public class GameLevel extends InputAdapter implements Screen {
	...
	public void render(float delta) {
		...
		if ((dizzy.status & Dizzy.PAUSED) > 0) {
			if (Gdx.input.justTouched()) {
				// Move Dizzy if paused and user clicks
				dizzy.status = dizzy.status & ~Dizzy.PAUSED;
			}
		} else {
			if (Gdx.input.isTouched()) {
				...
				isTouched = false;
				Gdx.app.log("INFO",
						"Before: " + Float.toString(dizzy.vector.angle()));
				dizzy.vector.setAngle(dizzy.getRotation());
				Gdx.app.log("INFO",
						"After: " + Float.toString(dizzy.vector.angle()));
				dizzy.status = dizzy.status | Dizzy.MOVING;
			} else {
		...
		font.draw(batch, dizzy.vector + " @ " + dizzy.vector.angle() + "/"
				+ dizzy.getRotation(), dizzy.getX(),
				dizzy.getY() + (2 * dizzy.getHeight()));
		batch.end();
	}
	...
	public void bounceDizzy() {
			...
			if (dizzy.intersectsWith(a)) {
				Gdx.app.log("INFO",
						"intersect at " + a.getName() + " @ " + a.getX() + "/"
								+ a.getY());
				dizzy.vector.scl(1.2f).clamp(1f, 5f);
				a.setVisible(false);
				a.remove();
			}
		...
		if (dizzy.getX() < 0
			...
			dizzy.vector.x *= -1;
			dizzy.vector.scl(0.95f).clamp(1f, 5f);
			if (dizzy.getX() < 0) {
				dizzy.setX(0);
			} else {
	...
	public void show() {
		...
		for (Image actor : actors) {
			if (actor.getName().equalsIgnoreCase("dizzy")) {
				...
				actor.setPosition((float) Math.random() * (stage.getWidth() - (2 * actor.getWidth())) + actor.getWidth(),
						(float) Math.random() * (stage.getHeight() - (2 * actor.getHeight())) + actor.getHeight());
			}
		}
	}
